...
public class MainForSpring05 {  //Ctx01.xml, Ctx02.xml 
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(selectMenu.equals("0")) {
				...
				processListCommand();
			}else if(selectMenu.equals("4")) {  // 특정 회원 목록 보기
				processInfoCommand();
	...
	private static void processChangeCommand() {
		...
		System.out.println("비밀번호를 변경할 계정(이메일)을 입력하세요.");
		String email = scan.nextLine();
		...
		//실제 비밀번호를 변경할 기능을 가진 객체를 불어와야 한다.
		...
		try {
			...
			System.out.println("존재하지 않는 이메일입닌다.");
