...
public class ActionFactory {
	...
	public Action getAction(String command){
		...
		if(command.equals("board_list")) {
			action = new BoardListAction(); // 게시물 전체 보기(목록보기)
		}else if(command.equals("board_write_form")) {
			action = new BoardWriteFormAction(); // 게시물 등록 폼 연결
		}else if(command.equals("board_write")) {
			action = new BoardWriteAction(); // 등록된 게시물을 DB에 저장하는 기능 
		}else if(command.equals("board_view")) {
			action = new BoardViewAction(); // 선택된 게시물 상세보기 기능
			...
			action = new BoardUpdateAction(); // 수정기능
		}
